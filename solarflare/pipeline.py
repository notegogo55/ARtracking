...
def plan_stages(sample_dir: Path, force: frozenset[str] = frozenset()) -> dict[str, str]:
    ...
    plan = {}
    plan["fetch"] = (
        "build" if "fetch" in force or not (sample_dir / "meta.json").exists() else "cached"
    )
    plan["segment"] = (
        "build"
        if "segment" in force
        or plan["fetch"] == "build"
        or not (sample_dir / "ar_masks.npy").exists()
        else "cached"
    )
    plan["features"] = (
        "build"
        if "features" in force
        or plan["segment"] == "build"
        or not (sample_dir / "features_frame.csv").exists()
        else "cached"
    )
...
def run_all(
    ...
    if plan["fetch"] == "build":
        if not email:
            raise ValueError(
                "stage A needs a JSOC-registered email (set JSOC_EMAIL) or an existing sample cache"
            )
    ...
    if plan["segment"] == "build":
        segment_sample_auto(sample, cfg.segment)
    ...
    record(
        ...
        **{
            k: stats[k]
            for k in ("n_sequences", "n_positive", "positive_rate", "missing_fraction_overall")
        },
    )
    ...
    per_fold, _ = crossval_table(
        X,
        rows["label"].to_numpy(dtype=int),
        rows["t0"],
        feature_names,
        horizon_steps=lookback_steps,
        n_folds=eval_folds,
        embargo_hours=eval_embargo_hours,
    ...
    record(
        ...
        tss_mean={r["model"]: round(float(r["tss_mean"]), 4) for _, r in table.iterrows()},
    )
...
def reproducibility_keys(manifest: dict) -> dict:
    ...
    return {
        ...
        "dataset": {
            k: v for k, v in manifest["stages"]["dataset"].items() if k not in ("seconds", "status")
        },
        "evaluate": {
            k: v
            for k, v in manifest["stages"]["evaluate"].items()
